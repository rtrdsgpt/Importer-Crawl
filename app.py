...
[Directory & Report Mining] -> Ranking -> [Validation]) end-to-end for a given
...
def run_pipeline(
    ...
    min_candidates: int = 15, max_expansion_rounds: int = 2,
) -> dict:
    slug = f"{slugify(product)}_{slugify(country)}"
    DATA_DIR.mkdir(exist_ok=True)
    provider = LABEL_TO_PROVIDER[provider_label]

    # Discovery, Scraping, and Ranking always run; Directory/Report Mining
    # and Validation are optional -- only count enabled phases so the overall
    ...
    with st.status("Phase 1: Discovering candidate companies...", expanded=True) as status:
        status.write(
            ...
            "English query variations" + (
                ...
                f" If fewer than {min_candidates} genuine candidates turn up, {provider_label} "
                "will be asked for supplementary queries (alternate terminology, trade fairs, "
                "local directories) and search runs again, up to "
                f"{max_expansion_rounds} extra round(s)." if min_candidates > 0 else ""
            )
        )
    ...
    with st.status("Phase 2: Scraping candidate pages...", expanded=True) as status:
        status.write(
            ...
            "and Facebook are never fetched either (against their ToS), but a company's "
            "title/snippet from either is still kept as low-confidence evidence -- useful "
            "for a smaller importer/wholesaler whose only real presence is a LinkedIn page "
            "or Facebook Page/Marketplace listing, not a separate website."
        )
    ...
    if mine_dirs:
        with st.status("Phase 3: Mining directory/report pages for more leads...", expanded=True) as status:
            status.write(
                f"B2B directory pages (europages, Kompass, etc.) and market research report "
                f"pages (Mordor Intelligence, Grand View Research, etc. -- reports often name "
                f"real \"key players\" active in this specific market) often list many company "
        ...
        advance_overall("Phase 3 (Directory & Report Mining) done")
...
def main() -> None:
    ...
    st.title("Importer Crawl")
    st.caption("AI-Powered Importer Discovery Engine for finding and ranking genuine importer companies for an Indian exporter entering a foreign market.")
    ...
    with st.sidebar:
        ...
        mine_dirs = st.checkbox("Mine directory/report pages for extra leads", value=False,
                                 help="Extracts company names from scraped B2B directory pages "
                                      "and market research reports (e.g. \"key players\" in a "
                                      "\"{product} market in {country}\" report), and searches "
                                      "for their real websites, using the same provider selected "
                                      "above. Adds significant runtime.")
        ...
        with st.expander("Advanced"):
            ...
            min_candidates = st.number_input(
                "Min genuine candidates before expanding search", 0, 100, 15,
                help="If Phase 1 finds fewer website/social candidates than this, the LLM is "
                     "asked for supplementary queries (alt terminology, trade fairs, local "
                     "directories) and search runs again. 0 disables this.",
            )
            max_expansion_rounds = st.number_input(
                "Max search expansion rounds", 0, 10, 2,
                help="How many times to retry with supplementary queries if still below the "
                     "minimum candidate count above.",
            )
    ...
    with tab_results:
        if run_clicked:
            if not api_key:
                ...
                run_output = run_pipeline(
                    ...
                    int(min_candidates), int(max_expansion_rounds),
                )
        ...
        if "last_run" in st.session_state:
            ...
            st.info(
                ...
                "*3. Directory/report mining* (optional: pull more leads out of B2B "
                "directory pages and market research reports) -> **4. Ranking** "
                "(an LLM judges genuine-importer role and "
